...
INVALID_SECTOR_NUMBER        = 0
VALID_SECTOR_NUMBER          = 1
UNSUCCESSFUL_ERASE           = 2
SUCCESSFUL_ERASE             = 3

FLASH_PAYLOAD_WRITE_FAILED   = 0
FLASH_PAYLOAD_WRITE_PASSED   = 1
...
def Read_Data_From_Serial_Port(Command_Code):
    ...
    if(len(BL_ACK)):
        ...
        elif (BL_ACK_Array[0] == 0xAB):
            print ("\n   Received Not-Acknowledgement from Bootloader")
            print ("BL_ACK_Array[0]",BL_ACK_Array[0])
            sys.exit()
...
def Process_CBL_GET_VER_CMD(Data_Len):
    ...
    print("   Bootloader Version   : ", (_value_[1] - 1), ".", (_value_[2] -1), ".", (_value_[3]-1))
...
def Process_CBL_FLASH_ERASE_CMD(Data_Len):
    ...
    if(len(Serial_Data)):
        ...
        if(BL_Erase_Status[0] == 0x01):
            print("\n   Erase Status -> Invalid Page Number ")
        elif (BL_Erase_Status[0] == 0x03):
            print("\n   Erase Status -> Unsuccessfule Erase ")
        elif (BL_Erase_Status[0] == 0x04):
            print("\n   Erase Status -> Successfule Erase ")
        else:
            print("\n   Erase Status -> Unknown Error")
            print(BL_Erase_Status[0])
    else:
...
def Decode_CBL_Command(Command):
    ...
    if(Command == 1):
        ...
        Write_Data_To_Serial_Port(BL_Host_Buffer[1], 1)
        sleep(0.01)
        ...
        SectorNumber = input("\n   Please enter start page number(0-31)          : ")
        SectorNumber = int(SectorNumber, 10)
        if(SectorNumber != 0xFF):
           
            NumberOfSectors = int(input("\n   Please enter number of pages to erase (32 Max): "))
        ...
        print("Remaining Bytes in Bin File : ", BinFileRemainingBytes)
...
while True:
    print("===================================")
    print("STM32F103 Custome BootLoader")
    print("===================================")
    print(" Send a command to the bootLoader :");
    print(" (1)   Get Version")
    print(" (2)   Get Help")
    print(" (3)   Get chip identification number")
    print(" (4)   Read the flash read protection level")
    print(" (5)   Jumb to address")
    print(" (6)   Erase Flash")
    print(" (7)   Write to Flash address")
    print(" (8)   Enable read write protect flash")
    print(" (9)   Read from flash")
    print(" (10)  Read page status")
    print(" (11)  OTP READ")
    print(" (12)  Disable read write protection on flash")
